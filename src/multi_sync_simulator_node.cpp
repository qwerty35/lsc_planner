...
int main(int argc, char* argv[]){
    ...
    if (param.multisim_replay) {
        Mission mission(nh);
        if (not mission.loadMission(param.multisim_max_noise, param.world_dimension,
                                    param.world_z_2d)) {
    ...
    Mission mission(nh);
    for(int si = 0; si < mission.mission_file_names.size(); si++){
        // Load world from octomap
        std::string world_file_name = "none";
        if(param.world_use_octomap){
            if(mission.world_file_names.size() == mission.mission_file_names.size()){
                world_file_name = mission.world_file_names[si];
            }
            else{
                ROS_WARN_STREAM_ONCE("The number of world file is not match to the number of mission file, use "
                                     << mission.world_file_names[0]);
                world_file_name = mission.world_file_names[0];
            }
        }
